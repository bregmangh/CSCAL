...
from models_adv import VideoModel
from loss import *
from cwj_autoparam_opts import parser
...
def main():
    ...
    args.use_target = 'uSv'
    args.exp_path = 'results/src_adv_'
    args.epochs = 130  # fault=30
    args.fc_dim = 512  # default: 512.
    args.resume = 'results/src_try_RGB/model_best.pth.tar'
    args.add_loss_DA = 'none'
    args.lr = 0.025  # 81.11%
    args.temperature = 6.0
    ...
    if args.tensorboard:
        writer = SummaryWriter(path_exp + '/runs')  # for tensorboardX
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
        ...
        if args.adv_DA != 'none' and args.use_target != 'none':
            ...
            for l in range(len(args.place_adv)):
                if args.place_adv[l] == 'Y':
                    ...
                    temperature = args.temperature # 1.8
                    src_pred = out_source.div(temperature)
                    src_soft_label = F.softmax(out_source, dim=1).detach()
                    src_soft_label[src_soft_label > 0.9] = 0.9
                    if l == 2:
                        src_soft_label = torch.repeat_interleave(src_soft_label, repeats=args.num_segments, axis=0)
                    if l == 0:
                        src_soft_label = torch.repeat_interleave(src_soft_label, repeats=4, axis=0)

                    tgt_pred = out_target.div(temperature)
                    tgt_soft_label = F.softmax(tgt_pred, dim=1).detach()
                    tgt_soft_label[tgt_soft_label > 0.9] = 0.9
                    if l == 2:
                        tgt_soft_label = torch.repeat_interleave(tgt_soft_label, repeats=args.num_segments, axis=0)
                    if l == 0:
                        tgt_soft_label = torch.repeat_interleave(tgt_soft_label, repeats=4, axis=0)

                    source_domain_label = torch.cat((src_soft_label, torch.zeros_like(src_soft_label)), dim=1)
                    target_domain_label = torch.cat((torch.zeros_like(tgt_soft_label), tgt_soft_label), dim=1)
                    # =========
                    ...
                    loss_adversarial_single = soft_label_cross_entropy(pred_domain, domain_label)
    ...
    if args.tensorboard and epoch==75:
...
def validate(val_loader, model, criterion, num_class, epoch, log):
    ...
    if args.tensorboard and epoch==75:  # update the embedding every iteration
        # embedding
...
def soft_label_cross_entropy(pred, soft_label):
    # N, C, dim = pred.shape
