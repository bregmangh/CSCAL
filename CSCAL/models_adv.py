...
class VideoModel(nn.Module):
    ...
    def _prepare_DA(self, num_class, base_model):  # convert the model to DA framework
        ...
        self.fc_feature_source = nn.Linear(feat_shared_dim, feat_frame_dim)  # Linear(in_features, out_features)
        ...
        # ===========
        self.fc_classifier_domain_1 = nn.Linear(feat_frame_dim, num_class)  # for source domain
        normal_(self.fc_classifier_domain_1.weight, 0, std)
        constant_(self.fc_classifier_domain_1.bias, 0)

        self.fc_classifier_domain_2 = nn.Linear(feat_frame_dim, num_class)  # for target domain
        normal_(self.fc_classifier_domain_2.weight, 0, std)
        constant_(self.fc_classifier_domain_2.bias, 0)
        # ===========
        ...
        # ==============
        self.fc_classifier_domain_video_1 = nn.Linear(feat_video_dim, num_class)  # for source domain
        normal_(self.fc_classifier_domain_video_1.weight, 0, std)
        constant_(self.fc_classifier_domain_video_1.bias, 0)

        self.fc_classifier_domain_video_2 = nn.Linear(feat_video_dim, num_class)  # for target domain
        normal_(self.fc_classifier_domain_video_2.weight, 0, std)
        constant_(self.fc_classifier_domain_video_2.bias, 0)
        # ==============
        ...
        if self.frame_aggregation == 'trn-m':
            ...
            # ==============
            self.relation_domain_classifier_all_1 = nn.ModuleList()  # for source domain
            for i in range(self.train_segments - 1):
                relation_domain_classifier_1 = nn.Sequential(
                    nn.Linear(feat_aggregated_dim, feat_video_dim),
                    nn.ReLU(),
                    nn.Linear(feat_video_dim, num_class)
                )
                self.relation_domain_classifier_all_1 += [relation_domain_classifier_1]

            self.relation_domain_classifier_all_2 = nn.ModuleList()  # for target domain
            for i in range(self.train_segments - 1):
                relation_domain_classifier_2 = nn.Sequential(
                    nn.Linear(feat_aggregated_dim, feat_video_dim),
                    nn.ReLU(),
                    nn.Linear(feat_video_dim, num_class)
                )
                self.relation_domain_classifier_all_2 += [relation_domain_classifier_2]
            # ==============
    ...
    def domain_classifier_frame(self, feat, beta):
        ...
        # ==========
        src_out = self.fc_classifier_domain_1(feat_fc_domain_frame)
        tgt_out = self.fc_classifier_domain_2(feat_fc_domain_frame)
        pred_fc_domain_frame = torch.cat((src_out, tgt_out), dim=1)
        # =========
    ...
    def domain_classifier_video(self, feat_video, beta):
        ...
        # ========
        src_out = self.fc_classifier_domain_video_1(feat_fc_domain_video)
        tgt_out = self.fc_classifier_domain_video_2(feat_fc_domain_video)
        pred_fc_domain_video = torch.cat((src_out, tgt_out), dim=1)
        # ========
    ...
    def domain_classifier_relation(self, feat_relation, beta):
        ...
        # ========
        # 128x4x256 --> (128x4)x(num_class)
        pred_fc_domain_relation_video = None
        for i in range(len(self.relation_domain_classifier_all)):
            feat_relation_single = feat_relation[:, i, :].squeeze(1)  # 128x1x256 --> 128x256
            feat_fc_domain_relation_single = GradReverse.apply(feat_relation_single, beta[0])  # the same beta for all relations (for now)
            src_out = self.relation_domain_classifier_all_1[i](feat_fc_domain_relation_single)
            tgt_out = self.relation_domain_classifier_all_2[i](feat_fc_domain_relation_single)
            num_class = src_out.size(-1)
            if pred_fc_domain_relation_video is None:
                pred_fc_domain_relation_single = torch.cat((src_out, tgt_out), 1)
                pred_fc_domain_relation_video = pred_fc_domain_relation_single.view(-1, 1, num_class*2)
            else:
                pred_fc_domain_relation_single = torch.cat((src_out, tgt_out), 1)
                pred_fc_domain_relation_video = torch.cat((pred_fc_domain_relation_video, pred_fc_domain_relation_single.view(-1, 1, num_class*2)), 1)

        pred_fc_domain_relation_video = pred_fc_domain_relation_video.view(-1, num_class*2)
        return pred_fc_domain_relation_video
        # ==========
    ...
    def forward(self, input_source, input_target, beta, mu, is_train, reverse):
        ...
        # === classifying branch (frame-level)
        ...
        if self.frame_aggregation == 'avgpool' or self.frame_aggregation == 'rnn':
            ...
            feat_fc_video_relation_source = self.TRN(feat_fc_video_source)  # [128,5,512] --> [128,4,256] (batch, all relations, 256-dim)
            feat_fc_video_relation_target = self.TRN(feat_fc_video_target)

            # adversarial branch (relations)
        ...
        # === classifying branch (video-level) ===#
        ...
        return attn_relation_source, output_source, output_source_2, pred_domain_all_source[::-1], feat_all_source[::-1], attn_relation_target, output_target, output_target_2, pred_domain_all_target[::-1], feat_all_target[
